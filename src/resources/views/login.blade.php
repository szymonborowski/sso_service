...
@section('content')
    <div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full space-y-8">
            <div>
                <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">{{ __('auth.login_heading') }}</h2>
                <p class="mt-2 text-center text-sm text-gray-600">{{ __('auth.no_account') }} <a href="{{ route('register') }}" class="font-medium text-indigo-600 hover:text-sky-800">{{ __('auth.register_link') }}</a></p>
            </div>
            ...
            <form method="POST" action="{{ url('/login')  }}" class="mt-8 space-y-6 bg-white p-6 rounded-lg shadow">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-sm">
                        @if (Route::has('password.request'))
                            <a class="font-medium text-indigo-600 hover:text-sky-800" href="{{ route('password.request') }}">{{ __('auth.forgot_password') }}</a>
                        @endif
                    </div>
                </div>
                ...
                {{--
                <div class="pt-4">
                  <div class="text-sm text-center text-gray-500 mb-2">{{ __('auth.or_login_with') }}</div>
